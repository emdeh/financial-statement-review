...
- **Azure Blob Storage:** All uploaded documents will be stored in Azure Blob Storage (this mimics Production). When a new file is uploaded to the designated container, it will trigger an Azure Function via Event Grid. The Blob filename or metadata will carry the charity ID or submission ID to track it.
