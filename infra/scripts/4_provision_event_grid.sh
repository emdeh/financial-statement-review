...
STORAGE_ACCOUNT_ID=$(az storage account show --name "$AZURE_STORAGE_ACCOUNT_NAME" --resource-group "$AZURE_RESOURCE_GROUP" --query "id" --output tsv)
...
az deployment group create \
    ...
    --parameters eventSubscriptionName="$AZURE_EVENT_GRID_SUBSCRIPTION_NAME" storageAccountId="$STORAGE_ACCOUNT_ID" azureFunctionResourceId="$FUNCTION_APP_RESOURCE_ID"
